...
const groq_sdk = require_rolldown_runtime.__toESM(require("groq-sdk"));
...
function convertMessagesToGroqParams(messages) {
	return messages.map((message) => {
		...
		if ((0, __langchain_core_messages.isAIMessage)(message) && !!message.tool_calls?.length) completionParam.tool_calls = message.tool_calls.map(__langchain_core_output_parsers_openai_tools.convertLangChainToolCallToOpenAI);
		else {
...
* <br />
*/
var ChatGroq = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	lc_namespace = [
		"langchain",
		"chat_models",
		"groq"
	...
	constructor(fields) {
		super(fields);
		const apiKey = fields.apiKey || (0, __langchain_core_utils_env.getEnvironmentVariable)("GROQ_API_KEY");
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		return __langchain_core_runnables.RunnableSequence.from([{ raw: llm }, parsedWithFallback]).withConfig({ runName: "ChatGroqStructuredOutput" });
	}
};
